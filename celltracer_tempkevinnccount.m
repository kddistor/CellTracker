...
op.local = 'cyto';                       %Indicate if nuclear/cytoplasmic
op.cname = {'CFP2','YFP-2', 'RFP2'};       %Channel names
...
fname = ['K:\',...
    '2015-07-07-mcf10a-tekar-FOXO.nd2']; 

     
%Save file name
sname = '2015-07-07-mcf10a-tekar-FOXO';
...
xyshift.frame = [54];   %First shifted frame
xyshift.dx = [-43];      %Magnitude of x shifts (x_after - x_before)
xyshift.dy = [9];      %Magnitude of y shifts (y_after - y_before)
...
bk(1).reg = [154,109,164]; bk(1).dyn = false; bk(1).fix = true;
bk(2:115) = bk(1);
...
bkmd.exp.Exposure   =   [100, 100, 200];        %Exposure times (ms)
bkmd.exp.ExVolt     =   [50, 50, 50];           %Relative voltages used
...
[r, ri, GMD, xtr] = iman_celltrace_parbfccount(p, r, ri);
