...
op.chan = 1;                            %Channel to use for segmenting
op.local = 'nuc';                       %Indicate if nuclear/cytoplasmic
op.cname = {'YFP', 'RFP'};       %Channel names
...
op.storemask = true;    %Store cells masks?  (may accumulate memory)
...
fname = ['F:\Testing\',...
    '2015-06-04-c2c12-erktr-fire_crop_crop2.nd2']; 
     
%Save file name
sname = '2015-06-04-c2c12-erktr-fire_crop_crop2';
...
% xyshift.frame = [1, 5];   %First shifted frame
% xyshift.dx = [-10; 20];      %Magnitude of x shifts (x_after - x_before)
% xyshift.dy = [-10; 20];      %Magnitude of y shifts (y_after - y_before)
...
clear bk
% s = 1;
% bk(s).reg =  [220, 17; 306, 120];       bk(s).fix = false;  s = s + 1;
% bk(s).reg =  [774, 710; 844, 785];      bk(s).fix = false;  s = s + 1;
% [bk(1:end).dyn] = deal(false);

% Note: for Fixed value background:
bk(1).reg = [157, 263]; bk(1).dyn = false; bk(1).fix = true;
% bk(2:60) = bk(1);
...
bkmd.exp.Channel    =   {'YFP2', 'RFP'};  %Name(s) of Channel(s)
bkmd.exp.Exposure   =   [700, 400];        %Exposure times (ms)
bkmd.exp.ExVolt     =   [40, 30];           %Relative voltages used
bkmd.exp.Filter     =   {'Filter_YFP', 'Filter_Cherry2'};
bkmd.exp.FPhore     =   {'YPet', 'mCherry'};
...
[r, ri, GMD, xtr] = iman_celltrace_parbf2(p, timeRange, r, ri);
% [r, ri, GMD, xtr] = iman_celltrace_parbf(p, r, ri);
