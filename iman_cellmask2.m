...
function [valcube, mask, gi] = iman_cellmask2(imin, coord, op, bkg, cellmasks)
...
%Keep only masks that have match track coordinates
for i=1:numel(lind)
    lind(i)=sort(lind(i));
    for j=1:numel(cellmasks.PixelIdxList)
        if ismember(lind(i), cellmasks.PixelIdxList{j})
            ctmatch{i}=cellmasks(1).PixelIdxList{j};
            cellmasks(1).PixelIdxList{j}=[];
            break
        else
            continue
        end
...
%Create binary image of all identified masks 
ctm=zeros(cellmasks(1).ImageSize);
for i=1:numel(ctmatch)
    for j=1:numel(ctmatch{i})
        ctm(ctmatch{i}(j))=1;
    end
end
